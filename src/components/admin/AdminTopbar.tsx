...
import { Search, Eye, UploadCloud, CheckCircle2, ChevronDown, LogOut } from "lucide-react";
...
export function AdminTopbar({ profile, draftCount }: { profile: Profile; draftCount: number }) {
  ...
  return (
    <header className="flex h-16 shrink-0 items-center gap-4 border-b border-paper/10 bg-ink px-6">
      ...
      <button
        ...
        disabled={isPending}
        className="flex items-center gap-1.5 rounded-full border border-paper/20 px-4 py-2 text-sm text-paper transition-colors hover:border-paper/40 disabled:opacity-60"
      >
      ...
      <form action={publishAction}>
        <button
          ...
          className="flex items-center gap-1.5 rounded-full bg-red px-4 py-2 text-sm font-medium text-paper transition-colors hover:bg-red-dark disabled:opacity-40"
        >
          {publishState.status === "success" ? (
            ...
            <>
              <UploadCloud size={15} /> {publishPending ? "Wird veröffentlicht …" : "Veröffentlichen"}
            </>
          )}
        </button>
      </form>
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 rounded-full py-1 pl-1 pr-2 text-sm text-paper hover:bg-paper/5"
