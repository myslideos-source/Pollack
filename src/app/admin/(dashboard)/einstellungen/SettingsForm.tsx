...
import { saveNotificationEmailAction } from "@/app/admin/actions/settings";

export function SettingsForm({ notificationEmail }: { notificationEmail: string }) {
  ...
  function submit(formData: FormData) {
    startTransition(async () => {
      const res = await saveNotificationEmailAction(formData);
      if (res.error) {
        setError(res.error);
        setSaved(false);
      } else {
  ...
  return (
    <form action={submit} className="max-w-md space-y-3 rounded-2xl border border-paper/10 bg-anthracite p-5">
      <div>
        <label className="mb-1 block text-xs text-paper/50">Benachrichtigungs-E-Mail-Adresse</label>
        <input
        ...
        />
        <p className="mt-1 text-xs text-paper/40">An diese Adresse geht die Benachrichtigung bei jeder neuen Anfrage.</p>
      </div>
