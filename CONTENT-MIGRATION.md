...
stattdessen in vier Lieferungen elf echte Fotos und ein Video direkt im Chat bereitgestellt
(Hero-/Beratungsszene, Fitness ×2, FIVE-Bereich, Kinderkarate, Solarium, Hardcore Area,
Gruppenfoto, Karate-Porträt, MORE-Nutrition-Produktregal, Karate-Trainingsvideo) — diese sind
bereits eingebaut, siehe Tabelle unten und MEDIA_AUDIT.md (inkl. der Anmerkungen zum
Hero-Bild-Wechsel und zu zwei erkannten Doppel-Uploads). Für alle übrigen Bildflächen (Milon,
InBody, Selbstverteidigung, Massage, Yoga, Jürgen Pollack im Porträt sofern nicht das
Karate-Foto dafür bestätigt wird, ESN-Produktbild) zeigt die Seite weiterhin die abstrakte
`TexturePanel`-Platzhalterkomponente. Sobald weiteres Material vorliegt, richtet sich der Einbau
nach der Checkliste in MEDIA_AUDIT.md.
...
| `/more-und-esn` | MORE Nutrition- und ESN-Produkte, Produktbilder, Produktvideo | `/partner-produkte` (Redirect von `/more-und-esn` und `/regeneration/more-nutrition-esn`) | Editorial-Bereich mit MORE-Nutrition- und ESN-Sektion, „bei uns im Sportpark erhältlich"-Hinweis, kein Online-Verkauf | Foto: MORE-Nutrition-„Chunky"-Dosen im Regal (`more-nutrition.webp`) als Aufmacherbild, auch im Startseiten-Teaser | Keine (Produktvideo weiterhin ausstehend) | Sprachlich komplett neu geschrieben, gekürzt, in Du-Ansprache, kein 1:1-Kopieren der Altseite | Weitere Produktbilder (v. a. ESN), Produktvideo |
...
| `/programm/karate` | Karate: Anfänger/Fortgeschrittene, Kondition, Kraft, Schnelligkeit, Beweglichkeit, Konzentration, Respekt, Jürgen-Pollack-Erfahrung, 2. DAN, 200+ Lehrgänge, Boxen/Kickboxen/Krav Maga/Bodenkampf/Kyusho | `/kampfkunst/karate` | Alle genannten Trainingsinhalte und Jürgen-Pollack-Qualifikationen übernommen; Hinweis auf kommendes Kickboxen-Angebot (Erweiterung 2026) auf der Kampfkunst-Übersicht ergänzt | Foto: Karate-Porträt in Kampfstellung (`karate-portrait.webp`) — Identität nicht bestätigt, siehe TODO_CLIENT.md #2a | Video: 12-Sekunden-Trainingsclip (Schlagtechniken gegen Kick-Pads), click-to-play, kein Autoplay | Deutlich erweitert, erstes echtes Video der Seite | Bestätigung der Personen-Identität; Nutzungsrechte für Foto und Video |
...
| Wurden die vorhandenen Produktbilder verwendet? | ⚠️ Teilweise — ein MORE-Nutrition-Produktfoto wurde nachgeliefert und ist eingebaut (`/partner-produkte`, Startseite); ein ESN-spezifisches Bild fehlt noch |
| Wurde das vorhandene Produktvideo eingebunden? | ❌ Noch nicht — das MORE-/ESN-Produktvideo wurde bisher nicht hochgeladen (das gelieferte Video zeigt Karate-Training und ist stattdessen auf `/kampfkunst/karate` eingebunden) |
